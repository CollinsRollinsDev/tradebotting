...
const CalcSurface = ({ props }) => {
  const selectedInterval = useSelector(
    (state) => state.selectedIntervalTD_Reducer.selectedInterval
  );
  ...
  const selectedTrade = useSelector(
    (state) => state.selectedTradeReducer.selectedTrade
  );
  const historyData = useSelector(
    (state) => state.historicDataForTradeReducer.historicDataForTrade
  );
  const selectIntervalFromUser = useSelector(
    (state) => state.selectedIntervalTD_Reducer.selectedInterval
  );
  ...
  useMemo(async () => {
    ...
    statements(selectedTrade.name, selectIntervalFromUser, percentageRange);
  }, [historyData]);
  ...
  return (
    <>
      <div className={styles.comPredition}>
        <p>{statement}</p>
